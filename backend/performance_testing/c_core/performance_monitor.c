...
static EnhancedPerformanceMetrics_t active_monitors[2048];
...
EnhancedPerformanceMetrics_t* stop_performance_monitoring_enhanced(int64_t handle) {
    ...
    active_monitor_slots[slot] = 0;  // Free the slot
    return metrics;
}

/**
 * @brief Free metrics structure (no-op for stack allocation)
...
int estimate_n_plus_one_cause(EnhancedPerformanceMetrics_t* metrics) {
    ...
    // Raised threshold - 5-6 queries is acceptable for Django apps with user profiles
    if (query_count < 12) return 0;
