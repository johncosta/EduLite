...
class QueryPattern:
    ...
    def is_n_plus_one(self) -> bool:
        ...
        same_table_count = sum(
            1 for query in self.related_queries 
            if table_name.lower() in query.lower() and 
               re.search(r'WHERE.*\bid\s*=\s*["\\]?\d+["\\]?', query, re.IGNORECASE)
        )
...
class DjangoAnalysisEngine:
    ...
    def detect_n_plus_one_queries(self, queries: List[Dict[str, Any]]) -> List[NplusOneDetection]:
        ...
        return [self._create_n_plus_one_detection(p, query_analyses) for p in patterns if p.is_n_plus_one()]
    ...
    def _group_queries_by_pattern(self, analyses: List[QueryAnalysis]) -> List[QueryPattern]:
        ...
        patterns = []
        base_queries = [a for a in analyses if a.operation == 'SELECT' and not self._is_single_row_lookup(a.sql)]
        
        for i, base_query in enumerate(base_queries):
            related_queries = []
            for subsequent_query in analyses[i + 1:]:
                if subsequent_query.operation == 'SELECT':
                    if self._is_single_row_lookup(subsequent_query.sql) and self._queries_likely_related(base_query, subsequent_query):
    ...
    def _queries_likely_related(self, base_query: QueryAnalysis, lookup_query: QueryAnalysis) -> bool:
        ...
        if base_table == "user":
            related_patterns.extend(["profile", "userprofile"])
        return any(p and p in lookup_table for p in related_patterns)
    
    def _create_n_plus_one_detection(self, pattern: QueryPattern, analyses: List[QueryAnalysis]) -> NplusOneDetection:
        """Creates an N+1 detection result object from a query pattern."""
        severity = "low"
        if pattern.count >= 50: severity = "critical"
        elif pattern.count >= 20: severity = "high"
        elif pattern.count >= 10: severity = "medium"
        ...
        if affected_tables:
            ...
            suggested_fix = "Consider using `select_related()` or `prefetch_related()` to reduce query count."
        
        return NplusOneDetection(
            detected=True, pattern_type="classic_n_plus_one",
            queries=[a for a in analyses if a.sql in pattern.related_queries],
            suggested_fix=suggested_fix, severity=severity, affected_tables=affected_tables
        )
    ...
    def _is_potentially_problematic(self, sql: str, duration: float) -> bool:
        ...
        if duration > 100:
            return True
        if sql.strip().upper().startswith('SELECT') and 'WHERE' not in sql.upper() and 'LIMIT' not in sql.upper():
            return True
    ...
    def generate_optimization_report(self, detections: List[NplusOneDetection]) -> str:
        ...
        """
        if not detections:
            return colors.colorize("✅ No N+1 query patterns detected!", EduLiteColorScheme.SUCCESS, bold=True)
        
        lines = [colors.colorize("🔍 Django Query Analysis Report", EduLiteColorScheme.ACCENT, bold=True), "=" * 50]
        severity_map = {
            'low': (EduLiteColorScheme.WARNING, '⚠️'), 'medium': (EduLiteColorScheme.SLOW, '🐌'),
            'high': (EduLiteColorScheme.CRITICAL, '🚨'), 'critical': (EduLiteColorScheme.CRITICAL, '💥')
        }
        ...
        if hasattr(connection, 'queries'):
            ...
            for query in django_queries:
                captured_queries.append({
                    'sql': query['sql'], 'duration': float(query['time']) * 1000,
                    'params': [], 'timestamp': 0
                })
