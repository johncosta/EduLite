...
# Load the enhanced C library
current_dir = Path(__file__).parent
lib_path = current_dir.parent / "c_core" / "libperformance.so"

if not lib_path.exists():
    raise FileNotFoundError(f"Performance library not found at {lib_path}")
...
try:
    ...
    print(f"--\t-- Warning: Enhanced performance monitoring functions not available in C library: {e}")
...
class DjangoPerformanceIssues:
    ...
    def get_issue_summary(self) -> List[str]:
        ...
        if self.has_n_plus_one:
            issues.append(f"🚨 N+1 Queries: {self.n_plus_one_analysis.severity_text} ({self.n_plus_one_analysis.query_count} queries)")
...
class EnhancedPerformanceMetrics_Python:
    ...
    def __init__(self, c_metrics: ctypes.POINTER(EnhancedPerformanceMetrics), operation_name: str) -> None:
        self.operation_name = operation_name
        
        # Basic metrics
        self.response_time = lib.get_elapsed_time_ms(c_metrics)
        self.memory_usage = lib.get_memory_usage_mb(c_metrics)
        self.memory_delta = lib.get_memory_delta_mb(c_metrics)
        ...
        # Other metrics
        self.query_count = lib.get_query_count(c_metrics)
        self.cache_hits = lib.get_cache_hit_count(c_metrics)
        self.cache_misses = lib.get_cache_miss_count(c_metrics)
        self.cache_hit_ratio = lib.get_cache_hit_ratio(c_metrics)
    ...
    def _analyze_django_issues(self, c_metrics: ctypes.POINTER(EnhancedPerformanceMetrics)) -> DjangoPerformanceIssues:
        """Analyze Django-specific performance issues with enhanced N+1 detection."""
        
        # Advanced N+1 analysis
        estimated_cause = lib.estimate_n_plus_one_cause(c_metrics)
        fix_suggestion = lib.get_n_plus_one_fix_suggestion(c_metrics).decode('utf-8')
        ...
        n_plus_one_analysis = NPlusOneAnalysis(
            has_severe=bool(lib.detect_n_plus_one_severe(c_metrics)),
            has_moderate=bool(lib.detect_n_plus_one_moderate(c_metrics)),
            has_pattern=bool(lib.detect_n_plus_one_pattern_by_count(c_metrics)),
            severity_level=lib.calculate_n_plus_one_severity(c_metrics),
        ...
        return DjangoPerformanceIssues(
            has_n_plus_one=bool(lib.has_n_plus_one_pattern(c_metrics)),
            excessive_queries=self.query_count > 20,
            memory_intensive=bool(lib.is_memory_intensive(c_metrics)),
            poor_cache_performance=bool(lib.has_poor_cache_performance(c_metrics)),
    ...
    def _detect_slow_serialization(self) -> bool:
        ...
        Returns:
            True if slow serialization is detected, False otherwise
        """
    ...
    def _detect_inefficient_pagination(self) -> bool:
        ...
        Returns:
            True if inefficient pagination is detected, False otherwise
        """
    ...
    def _detect_missing_indexes(self) -> bool:
        ...
        Returns:
            True if missing indexes are suspected, False otherwise
        """
    ...
    def _get_recommendations(self) -> List[str]:
        ...
        if self.django_issues.has_n_plus_one:
            ...
            if analysis.estimated_cause == 1:  # Serializer N+1
                ...
                recommendations.append("Consider using @property methods on models instead of SerializerMethodField")
                ...
                recommendations.append("Add prefetch_related() for reverse foreign keys and many-to-many")
                ...
                recommendations.append("Consider flattening data structure or using prefetch_related")
        ...
        if self.django_issues.excessive_queries and not self.django_issues.has_n_plus_one:
            recommendations.append("Consider query optimization, caching, or reducing database calls")
        
        if self.django_issues.memory_intensive:
            recommendations.append("Review queryset size, use pagination, or implement lazy loading")
        
        if self.django_issues.poor_cache_performance:
            recommendations.append("Review cache strategy, increase cache timeouts, or add missing cache keys")
        
        if self.django_issues.slow_serialization:
            recommendations.append("Optimize serializer fields, use SerializerMethodField sparingly")
    ...
    def _calculate_performance_score(self) -> PerformanceScore:
        ...
        points_lost, points_gained, optimization_impact = self._generate_score_explanations(
            response_time_score, query_efficiency_score, memory_efficiency_score,
            cache_performance_score, n_plus_one_penalty
        )
    ...
    def calculate_memory_payload_efficiency(self, response_size_kb: float):
        ...
        if response_size_kb > 0:
            ...
            self.memory_per_kb_payload = self.memory_overhead / response_size_kb if response_size_kb > 0 else 0
            self.memory_efficiency_ratio = estimated_payload_memory_mb / self.memory_overhead if self.memory_overhead > 0 else 1.0
    ...
    def _generate_score_explanations(self, response_score, query_score, memory_score,
                                   cache_score, n_plus_one_penalty):
        ...
        if n_plus_one_penalty > 0:
            analysis = self.django_issues.n_plus_one_analysis
            points_lost.append(f"N+1 queries: -{n_plus_one_penalty:.1f}pts ({analysis.severity_text})")
            optimization_impact["n_plus_one_fix"] = n_plus_one_penalty
        
        # Cache performance
        if cache_score < 8:
            lost = 10 - cache_score
            points_lost.append(f"Cache performance: -{lost:.1f}pts ({self.cache_hit_ratio:.1%} hit ratio)")
        ...
        if score.points_lost:
            ...
            for loss in score.points_lost:
                ...
                if "Memory usage" in loss:
                    if self.memory_overhead < 5:
                        lines.append(f"   {colors.colorize('Memory overhead minimal - no significant penalty', EduLiteColorScheme.FADE)}")
                    elif self.memory_overhead < 15:
                        lines.append(f"   {colors.colorize(f'Memory overhead {self.memory_overhead:.1f}MB - mostly temporary objects/payload processing', EduLiteColorScheme.FADE)}")
                    else:
                        lines.append(f"   {colors.colorize(f'Memory overhead {self.memory_overhead:.1f}MB - investigate potential memory leaks', EduLiteColorScheme.FADE)}")
                        if self.payload_size_kb > 0:
                            lines.append(f"   {colors.colorize(f'   Payload accounts for ~{self.memory_breakdown}', EduLiteColorScheme.FADE)}")
                else:
        ...
        if score.points_gained:
            ...
            for gain in score.points_gained:
                if "Memory usage" in gain:
                    lines.append(f"   {colors.colorize(f'Memory usage within reasonable Django baseline (~{self.baseline_memory_mb:.0f}MB)', EduLiteColorScheme.FADE)}")
                else:
...
class EnhancedPerformanceMonitor:
    ...
    def assert_performance(self, 
                          ...
                          min_cache_hit_ratio: Optional[float] = None) -> None:
        ...
        if max_response_time and self._metrics.response_time > max_response_time:
            failures.append(f"response time {self._metrics.response_time:.2f}ms > {max_response_time}ms")
        ...
        if min_cache_hit_ratio and self._metrics.cache_hit_ratio < min_cache_hit_ratio:
            failures.append(f"cache hit ratio {self._metrics.cache_hit_ratio:.1%} < {min_cache_hit_ratio:.1%}")
    ...
    def __enter__(self) -> Self:
        ...
        if self.handle == -1:
            raise RuntimeError(f"Failed to start performance monitoring for {self.operation_name}")
    ...
    def __exit__(self, exc_type: Any, exc_val: Any, exc_tb: Any) -> None:
        ...
        if self.handle is not None:
            ...
            if c_metrics:
                # Create enhanced Python metrics wrapper
                self._metrics = EnhancedPerformanceMetrics_Python(c_metrics, self.operation_name)
    ...
    def _assert_thresholds(self) -> None:
        ...
        if 'response_time' in self._thresholds:
            if self._metrics.response_time > self._thresholds['response_time']:
                violations.append(f"Response time {self._metrics.response_time:.2f}ms > {self._thresholds['response_time']}ms")
        
        if 'memory_usage' in self._thresholds:
            if self._metrics.memory_usage > self._thresholds['memory_usage']:
                violations.append(f"Memory usage {self._metrics.memory_usage:.2f}MB > {self._thresholds['memory_usage']}MB")
        
        if 'query_count' in self._thresholds:
            if self._metrics.query_count > self._thresholds['query_count']:
                violations.append(f"Query count {self._metrics.query_count} > {self._thresholds['query_count']}")
        
        if 'cache_hit_ratio' in self._thresholds:
            if self._metrics.cache_hit_ratio < self._thresholds['cache_hit_ratio']:
                violations.append(f"Cache hit ratio {self._metrics.cache_hit_ratio:.1%} < {self._thresholds['cache_hit_ratio']:.1%}")
        
        if violations:
            raise AssertionError(f"Performance thresholds exceeded: {'; '.join(violations)}")
...
def monitor_django_view(operation_name: str, operation_type: str = "view") -> EnhancedPerformanceMonitor:
    """
