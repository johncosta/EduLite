...
class MercuryConfiguration:
    ...
    regression_threshold_percent: float = 50.0
    regression_lookback_tests: int = 10
...
class MercuryConfigurationManager:
    ...
    def save_configuration(self, config: MercuryConfiguration) -> None:
        ...
        if 'thresholds' in data:
            data['thresholds'] = {
                op_type: asdict(threshold_obj) if hasattr(threshold_obj, '__dict__') else threshold_obj
...
class ProjectPerformanceStandards:
    ...
    def __init__(self):
        ...
        self.standards = {
            'response_time_goals': {'excellent': 50, 'good': 100, 'acceptable': 300, 'poor': 500, 'critical': float('inf')},
            'database_query_goals': {'excellent': 1, 'good': 3, 'acceptable': 5, 'poor': 10, 'critical': float('inf')},
            'memory_efficiency_goals': {'excellent': 10, 'good': 20, 'acceptable': 40, 'poor': 80, 'critical': float('inf')},
...
if __name__ == "__main__":
    ...
    print(f"Production response time threshold for list views: {prod_config.thresholds['list_view'].response_time_ms:.2f}ms")
