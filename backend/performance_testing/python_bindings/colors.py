...
class PerformanceColors:
    ...
    def _detect_color_support(self) -> bool:
        ...
        if self.mode == ColorMode.NEVER: return False
        if self.mode in [ColorMode.ALWAYS, ColorMode.RICH]: return True
        if os.environ.get('FORCE_COLOR') or os.environ.get('CLICOLOR_FORCE'): return True
        if os.environ.get('NO_COLOR') or os.environ.get('CLICOLOR') == '0': return False
    ...
    def colorize(self, text: str, color: str, bold: bool = False) -> str:
        ...
        """
        if not self._supports_color: return text
        return self._colorize_rich(text, color, bold) if self.mode == ColorMode.RICH else self._colorize_ansi(text, color, bold)
    ...
    def trend_color(self, trend: str) -> str:
        ...
        """
        trend_map = {
            'up': EduLiteColorScheme.TRENDING_UP, 'down': EduLiteColorScheme.TRENDING_DOWN,
            'stable': EduLiteColorScheme.TRENDING_STABLE, 'improving': EduLiteColorScheme.TRENDING_DOWN,
    ...
    def memory_color(self, memory_mb: float) -> str:
        ...
        if memory_mb <= 20: return EduLiteColorScheme.MEMORY_EXCELLENT
        if memory_mb <= 50: return EduLiteColorScheme.MEMORY_GOOD
        if memory_mb <= 100: return EduLiteColorScheme.MEMORY_WARNING
    ...
    def query_color(self, query_count: int) -> str:
        ...
        if query_count <= 3: return EduLiteColorScheme.QUERY_EFFICIENT
        if query_count <= 7: return EduLiteColorScheme.QUERY_ACCEPTABLE
        if query_count <= 15: return EduLiteColorScheme.QUERY_INEFFICIENT
    ...
    def format_metric_value(self, value: float, unit: str, threshold: Optional[float] = None) -> str:
        """
        ...
        if unit_lower in ['ms', 'milliseconds']:
            if value <= 50: color = EduLiteColorScheme.EXCELLENT
            elif value <= 100: color = EduLiteColorScheme.GOOD
            elif value <= 300: color = EduLiteColorScheme.ACCEPTABLE
            elif value <= 500: color = EduLiteColorScheme.SLOW
            else: color = EduLiteColorScheme.CRITICAL
        elif unit_lower in ['mb', 'megabytes']: color = self.memory_color(value)
        elif unit_lower in ['queries', 'query', 'q']: color = self.query_color(int(value))
        
        if threshold is not None:
            if value <= threshold * 0.7: color = EduLiteColorScheme.EXCELLENT
            elif value <= threshold: color = EduLiteColorScheme.GOOD
            elif value <= threshold * 1.2: color = EduLiteColorScheme.WARNING
            else: color = EduLiteColorScheme.CRITICAL
...
def get_status_icon(status: str) -> str:
    ...
    """
    status_icons = {
        'excellent': '🚀', 'good': '✅', 'acceptable': '⚠️', 'warning': '⚠️',
        'slow': '🐌', 'critical': '🚨', 'success': '🎯', 'info': '💡',
        'optimization': '🔧', 'trending_up': '📈', 'trending_down': '📉',
        'trending_stable': '➡️'
    }
...
if __name__ == "__main__":
    ...
    print(f"\n🎨 Palette Preview:")
    print(f"  - Excellence: {colors.colorize(EduLiteColorScheme.EXCELLENT, EduLiteColorScheme.EXCELLENT)}")
    ...
    print(f"  - Critical: {colors.colorize(EduLiteColorScheme.CRITICAL, EduLiteColorScheme.CRITICAL)}")