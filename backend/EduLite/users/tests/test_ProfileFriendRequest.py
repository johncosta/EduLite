...
class ProfileFriendRequestModelTests(TestCase):
    ...
    def test_clean_method_prevents_request_if_already_friends(self):
        ...
        self.assertTrue(self.user2.profile.friends.filter(pk=self.profile1.pk).exists()) # Check reverse via related_name

        # Attempt to create a friend request from profile1 to profile2
        request_to_friend = ProfileFriendRequest(sender=self.profile1, receiver=self.profile2)
        with self.assertRaisesMessage(ValidationError, "Cannot send a friend request to a friend."):
            request_to_friend.full_clean()

        # Test the other direction as well (profile2 trying to send to profile1)
        request_from_friend = ProfileFriendRequest(sender=self.profile2, receiver=self.profile1)
        with self.assertRaisesMessage(ValidationError, "Cannot send a friend request to a friend."):
    ...
    def test_clean_method_allows_request_if_not_friends(self):
        ...
        request_to_non_friend = ProfileFriendRequest(sender=self.profile1, receiver=self.profile3)
        try:
...
class UserProfileFriendRequestInteractionTests(TestCase): # Or APITestCase if making API calls later

    @classmethod
    def setUpTestData(cls):
        # Users
        cls.user_sender = User.objects.create_user(username='sender_user', password='password123')
        cls.user_receiver1 = User.objects.create_user(username='receiver1_user', password='password123')
        cls.user_receiver2 = User.objects.create_user(username='receiver2_user', password='password123')
        cls.user_another_sender = User.objects.create_user(username='another_sender', password='password123')
        ...
        cls.profile_another_sender = UserProfile.objects.get(user=cls.user_another_sender)

        # Create some friend requests
        # sender sends to receiver1 and receiver2
        ProfileFriendRequest.objects.create(sender=cls.profile_sender, receiver=cls.profile_receiver1)
        ProfileFriendRequest.objects.create(sender=cls.profile_sender, receiver=cls.profile_receiver2)

        # another_sender sends to receiver1
        ProfileFriendRequest.objects.create(sender=cls.profile_another_sender, receiver=cls.profile_receiver1)
    ...
    def test_filter_on_related_requests(self):
        ...
        specific_request_qs = self.profile_receiver1.received_friend_requests.filter(sender=self.profile_sender)
        ...
        has_sent_request = self.profile_sender.sent_friend_requests.filter(receiver=self.profile_receiver1).exists()
        self.assertTrue(has_sent_request)

        has_sent_to_non_receiver = self.profile_sender.sent_friend_requests.filter(receiver=self.profile_another_sender).exists()
    ...
    def test_related_name_after_request_accepted(self):
        ...
        """
        request_to_accept = self.profile_sender.sent_friend_requests.get(receiver=self.profile_receiver1)
        ...
        self.assertEqual(self.profile_sender.sent_friend_requests.count(), 1) # One less sent
        self.assertEqual(self.profile_receiver1.received_friend_requests.count(), 1) # One less received

        # Verify they are friends
        self.assertTrue(self.profile_sender.friends.filter(pk=self.user_receiver1.pk).exists())
        self.assertTrue(self.profile_receiver1.friends.filter(pk=self.user_sender.pk).exists())
    ...
    def test_related_name_after_request_declined(self):
        ...
        """
        request_to_decline = self.profile_sender.sent_friend_requests.get(receiver=self.profile_receiver2)

        # Before decline
        self.assertEqual(self.profile_sender.sent_friend_requests.count(), 2) # Assuming this test runs after setUpTestData
        # To make it isolated, re-fetch count based on initial setup or setup specific request for this test
        initial_sent_count_for_sender = ProfileFriendRequest.objects.filter(sender=self.profile_sender).count()
        initial_received_count_for_receiver2 = ProfileFriendRequest.objects.filter(receiver=self.profile_receiver2).count()
        ...
        # After decline
        self.assertEqual(self.profile_sender.sent_friend_requests.count(), initial_sent_count_for_sender - 1)
        self.assertEqual(self.profile_receiver2.received_friend_requests.count(), initial_received_count_for_receiver2 - 1)

        # Verify they are NOT friends
        self.assertFalse(self.profile_sender.friends.filter(pk=self.user_receiver2.pk).exists())
        self.assertFalse(self.profile_receiver2.friends.filter(pk=self.user_sender.pk).exists())
