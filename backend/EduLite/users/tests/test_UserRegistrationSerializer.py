...
from django.core.exceptions import ValidationError as DjangoValidationError # For mocking password validator
from rest_framework.exceptions import ValidationError as DRFValidationError # What serializer raises
...
class UserRegistrationSerializerTests(TestCase):
    ...
    def test_successful_registration_all_fields(self):
        ...
        self.assertTrue(user.check_password(self.valid_data_full['password'])) # Verify password is set and hashed
    ...
    def test_password_mismatch(self):
        ...
        serializer = UserRegistrationSerializer(data=data)
        self.assertFalse(serializer.is_valid())
        self.assertIn('password2', serializer.errors) # The error is attached to password2 by your validate method
    ...
    def test_duplicate_username(self):
        ...
        User.objects.create_user(username=self.valid_data_minimal['username'], password="somepassword")
        serializer = UserRegistrationSerializer(data=self.valid_data_minimal)
        self.assertFalse(serializer.is_valid())
        self.assertIn('username', serializer.errors)
    ...
    def test_duplicate_email(self):
        ...
        User.objects.create_user(username="another_user", email=self.valid_data_minimal['email'], password="somepassword")
        serializer = UserRegistrationSerializer(data=self.valid_data_minimal)
        self.assertFalse(serializer.is_valid())
        self.assertIn('email', serializer.errors)
    ...
    def test_email_invalid_format_missing_at_symbol_safeguard(self):
        ...
        self.assertTrue(
            "Enter a valid email address." in error_messages or \
            "Invalid email format: '@' symbol missing." in error_messages
        )
    ...
    def test_email_case_insensitivity_check(self):
        ...
        User.objects.create_user(username="another_user2", email="TestEmail@ExampleValid.com", password="password123")
        data = self.valid_data_minimal.copy()
