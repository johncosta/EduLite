...
class UserListViewTests(APITestCase):
    ...
    def test_list_users_as_admin_shows_all_users(self):
        ...
        self.assertIn('count', response.data, "Response should contain a 'count' field for pagination.")
        self.assertIn('results', response.data, "Response should contain a 'results' field for paginated data.")
        ...
        returned_usernames = sorted([item['username'] for item in response.data['results']])
        expected_usernames = sorted([self.admin_user.username, self.user1.username, self.user2.username, self.inactive_user.username])
        ...
        if len(response.data['results']) == self.total_users_in_db:
             self.assertListEqual(returned_usernames, expected_usernames,
