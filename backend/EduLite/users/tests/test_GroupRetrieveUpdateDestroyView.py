...
class GroupRetrieveUpdateDestroyViewTests(APITestCase):
    ...
    def setUpTestData(cls):
        # Create users
        cls.non_admin_user = User.objects.create_user(username='testuser_rud', password='password123')
        cls.admin_user = User.objects.create_superuser(username='admin_rud', password='password123')

        # Create a group to be manipulated in tests
        cls.group_to_modify = Group.objects.create(name='Marketing Team')
        cls.group_to_modify_url = reverse('group-detail', kwargs={'pk': cls.group_to_modify.pk})

        cls.non_existent_group_pk = cls.group_to_modify.pk + 100 # A PK that likely doesn't exist
        cls.non_existent_group_url = reverse('group-detail', kwargs={'pk': cls.non_existent_group_pk})
    ...
    def _get_jwt_token(self, username, password):
        ...
        self.assertEqual(response.status_code, status.HTTP_200_OK, f"Failed to obtain token for {username}")
    ...
    def test_retrieve_non_existent_group_returns_404(self):
        token = self._get_jwt_token('testuser_rud', 'password123') # Any authenticated user
        self.client.credentials(HTTP_AUTHORIZATION=f'Bearer {token}')
    ...
    def test_admin_user_can_put_group_with_valid_data(self):
        ...
        data = {'name': new_name} # GroupSerializer only has 'name' and 'url' (read-only) as primary fields
        response = self.client.put(self.group_to_modify_url, data, format='json')
        self.assertEqual(response.status_code, status.HTTP_200_OK)
    ...
    def test_authenticated_non_admin_cannot_delete_group(self):
        ...
        response = self.client.delete(self.group_to_modify_url)
        self.assertEqual(response.status_code, status.HTTP_403_FORBIDDEN)
        self.assertTrue(Group.objects.filter(pk=self.group_to_modify.pk).exists()) # Ensure not deleted
    ...
    def test_admin_user_can_delete_group(self):
        ...
        self.assertFalse(Group.objects.filter(pk=self.group_to_modify.pk).exists()) # Ensure deleted
