...
from rest_framework.test import APIRequestFactory # For providing request context to serializer
...
class ProfileFriendRequestSerializerTests(TestCase):
    ...
    def setUpTestData(cls):
        # Create users
        cls.user_sender = User.objects.create_user(username='sender', first_name='Sender', last_name='User', password='password123')
        cls.user_receiver = User.objects.create_user(username='receiver', first_name='Receiver', last_name='Person', password='password123')
    ...
    def _get_serializer_with_context(self, instance):
        ...
        request = self.factory.get(reverse('userprofile-detail', kwargs={'pk': self.profile_sender.pk})) # Dummy URL
    ...
    def test_created_at_field_content_and_format(self):
        ...
        self.assertRegex(serializer.data['created_at'], r'\d{4}-\d{2}-\d{2} \d{2}:\d{2}:\d{2}')
    ...
    def test_sender_profile_url_field_content(self):
        ...
        request_for_context = self.factory.get(reverse('userprofile-detail', kwargs={'pk': self.profile_sender.pk}))
        serializer = ProfileFriendRequestSerializer(self.friend_request, context={'request': request_for_context})
    ...
    def test_receiver_profile_url_field_content(self):
        ...
        request_for_context = self.factory.get(reverse('userprofile-detail', kwargs={'pk': self.profile_receiver.pk}))
        serializer = ProfileFriendRequestSerializer(self.friend_request, context={'request': request_for_context})
    ...
    def test_read_only_fields_are_not_writable(self):
        ...
        meta_read_only = getattr(ProfileFriendRequestSerializer.Meta, 'read_only_fields', [])
