...
class SendFriendRequestViewTest(UsersAppTestCase):
    ...
    def test_send_friend_request_requires_authentication(self):
        ...
        response = self._send_friend_request({'receiver_profile_id': self.marie.profile.id})
    ...
    def test_send_friend_request_success(self):
        ...
        friend_request = ProfileFriendRequest.objects.get(
            sender__user=self.ahmad,
            receiver__user=self.dmitri
        )
        self.assertEqual(friend_request.message, 'Hi! Let\'s connect and study together.')
