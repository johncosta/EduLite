...
class UserProfilePrivacySettingsRetrieveUpdateViewTest(DjangoMercuryAPITestCase):
    ...
    def test_update_with_empty_data(self):
        ...
        response = self.client.get(self.url)
        self.assertEqual(response.data['search_visibility'], original_data['search_visibility'])
    ...
    def test_update_with_null_values(self):
        ...
        if response.status_code == status.HTTP_200_OK:
            ...
            self.assertIn(self.ahmad.profile.privacy_settings.allow_friend_requests, [True, False])
        else:
            self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
    ...
    def test_privacy_settings_field_types(self):
        ...
        response = self.client.patch(self.url, {
            'show_email': 'invalid_bool'  # Should be boolean
        }, format='json')
