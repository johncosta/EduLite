...
class UserSerializerTest(TestCase):
    ...
    def test_serializer_contains_expected_fields(self):
        ...
        serializer = UserSerializer(
            instance=self.user,
            context=self.get_serializer_context(user=self.user)  # User viewing own profile
        )
        data = serializer.data
        
        expected_fields = [
            'id', 'url', 'profile_url', 'username', 'email',
            'groups', 'first_name', 'last_name', 'full_name'
        ]
    ...
    def test_email_hidden_based_on_privacy_settings(self):
        ...
        serializer = UserSerializer(
            instance=self.user,
            context=self.get_serializer_context(user=other_user)
        )
        data = serializer.data
    ...
    def test_email_visible_to_self(self):
        ...
        serializer = UserSerializer(
            instance=self.user,
            context=self.get_serializer_context(user=self.user)
        )
        data = serializer.data
