...
class UserUpdateDeleteViewTests(APITestCase):
    def setUp(self):
        ...
        self.detail_url = reverse('user-update', kwargs={'pk': self.target_user_for_edit.pk})
    ...
    def test_admin_update_user_put_invalid_data(self):
        self.client.force_authenticate(user=self.admin_user)
        invalid_data = {'email': 'notanemail'} # Username is required by PUT if not provided
        response = self.client.put(self.detail_url, invalid_data, format='json')
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
        self.assertIn('username', response.data) # PUT requires all fields for a full update
    ...
    def test_admin_can_partial_update_user_patch(self):
        ...
        self.assertEqual(self.target_user_for_edit.username, 'usertoedit') # Should remain unchanged
