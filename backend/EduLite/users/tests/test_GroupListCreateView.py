...
class GroupListCreateViewTests(APITestCase):
    ...
    def setUpTestData(cls):
        # Create users
        cls.non_admin_user = User.objects.create_user(username='testuser', password='password123')
    ...
    def test_authenticated_non_admin_can_list_groups(self):
        ...
        self.assertEqual(len(response.data['results']), 2) # Check if both groups are listed
        self.assertTrue(any(g['name'] == self.group1.name for g in response.data['results']))
