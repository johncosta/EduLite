...
class TestUserProfileModel(TestCase):
    ...
    def test_profile_auto_creation_on_user_create(self):
        ...
        with self.assertRaises(
            IntegrityError
        ):  # Or a more specific error if UserProfile.user is primary_key
    ...
    def test_profile_default_blank_null_fields(self):
        ...
        self.assertEqual(
            profile.bio, None
        )  # Or "" if you change null=True to default=""
        ...
        )  # Or "" if ImageField defaults to empty string
    ...
    def test_profile_can_save_choices_fields(self):
        ...
        profile = self.user1.profile
        profile.occupation = OCCUPATION_CHOICES[0][
            0
        ]  # Save the first value of occupation choices
    ...
    def test_profile_picture_field(self):
        ...
        updated_profile = UserProfile.objects.get(user=self.user1)
        self.assertTrue(
            updated_profile.picture.name.startswith("profile_pics/test_image")
        )
    ...
    def test_profile_friends_relationship(self):
        ...
        self.assertEqual(profile1.friends.count(), 0)
        self.assertNotIn(
            profile1, self.user2.friend_profiles.all()
        )  # Check reverse removal
