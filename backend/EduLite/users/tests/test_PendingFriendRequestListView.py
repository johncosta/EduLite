...
class PendingFriendRequestListViewTests(APITestCase):
    ...
    def setUpTestData(cls):
        ...
        ProfileFriendRequest.objects.create(sender=cls.profile_a, receiver=cls.profile_b)
        # 2. User C sends a request to User B
        ProfileFriendRequest.objects.create(sender=cls.profile_c, receiver=cls.profile_b)
        # 3. User B sends a request to User C
        ProfileFriendRequest.objects.create(sender=cls.profile_b, receiver=cls.profile_c)
    ...
    def test_list_is_paginated(self):
        ...
        for i in range(12):
            sender_user = User.objects.create_user(username=f'many_sender_{i}', password='password123')
            sender_profile = UserProfile.objects.get(user=sender_user)
            ProfileFriendRequest.objects.create(sender=sender_profile, receiver=self.profile_a)
