...
class UserRegistrationViewTests(APITestCase):
    ...
    def test_successful_user_registration_all_fields(self):
        ...
        self.assertTrue(created_user.is_active) # TODO: Setup email verification, or other means of account activation
        self.assertTrue(created_user.check_password(self.strong_password)) # Check password was set and hashed
    ...
    def test_successful_user_registration_minimal_fields(self):
        ...
        response = self.client.post(self.register_url, self.minimal_user_data, format='json')
        self.assertEqual(response.status_code, status.HTTP_201_CREATED, response.data)
        self.assertEqual(User.objects.count(), 1)
    ...
    def test_registration_fails_if_passwords_do_not_match(self):
        ...
        self.assertEqual(str(response.data['password2'][0]), "Password fields didn't match.")

    def test_registration_fails_for_duplicate_username(self):
        """Ensure registration fails if username already exists."""
        User.objects.create_user(username=self.user_data['username'], password='somepassword')
        response = self.client.post(self.register_url, self.user_data, format='json')
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
        self.assertIn('username', response.data)
    ...
    def test_registration_fails_for_duplicate_email(self):
        ...
        User.objects.create_user(username='anotheruser', email=self.user_data['email'], password='somepassword')
        response = self.client.post(self.register_url, self.user_data, format='json')
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
        self.assertIn('email', response.data)
        self.assertEqual(str(response.data['email'][0]), 'A user with this email address already exists.')

    def test_registration_fails_for_blocked_email_domain(self):
        """Ensure registration fails for an email from a blocked domain."""
        data = self.user_data.copy()
        data['email'] = 'user@example.com' # 'example.com' is blocked in your serializer
        response = self.client.post(self.register_url, data, format='json')
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
        self.assertIn('email', response.data)
        self.assertEqual(str(response.data['email'][0]), 'Registration from this email domain is not allowed.')
